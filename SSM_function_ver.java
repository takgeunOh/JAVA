...
public class SSM_function_ver {
	...
	public void search() {
		...
		while(true) {
			...
			if(duplicate_name_check(search_name)) {
				...
				choice_idx = same_name_print(save_index, count, search_name);
	...
	public void modify() {
		...
		while(true) {
			...
			if(duplicate_name_check(modify_name)) {
				// 동명이인이 2명 이상일 때
				System.out.printf("다음과 같이 %s 이름을 가진 학생이 둘 이상 존재합니다.\n", modify_name);
				System.out.printf("번호\t성명\t국어\t영어\t수학\t총점\t평균\t학점\t등수\n");

				// 동명이인의 인원수 구해서 그걸 출력범위로 사용해서 동명이인 리스트 출력할 것.
				count = duplicate_name_count(modify_name);
				
				// 원래 데이터에서 동명이인이 존재하는 인덱스 추출
				save_index = duplicate_name_idx(count, modify_name);
				
				// 동명이인 리스트 보여주고 번호 선택까지 한 후 해당 번호가 원래 데이터에서 몇 번 인덱스인지까지 구하기
				choice_idx = same_name_print(save_index, count, modify_name);
				
				// 이제부터는 선택한 학생의 원래 인덱스를 구하고 해당 인덱스의 성적을 모두 업데이트하기
				System.out.println("\n무엇을 수정하시겠습니까?");
				...
				if(modify_choice==1) {
					System.out.printf("%s학생 이름 수정 >> ", name[choice_idx]);
					name[choice_idx] = scan.next();
				}
				else if (modify_choice==2) {
					System.out.printf("%s학생 국어성적 수정 >> ", name[choice_idx]);
					kor[choice_idx] = scan.nextInt();
				}
				else if (modify_choice==3) {
					System.out.printf("%s학생 영어성적 수정 >> ", name[choice_idx]);
					eng[choice_idx] = scan.nextInt();
				}
				else if (modify_choice==4) {
					System.out.printf("%s학생 수학성적 수정 >> ", name[choice_idx]);
					math[choice_idx] = scan.nextInt();
				}
				System.out.println("choice_idx : " + choice_idx);
				// calc()는 top번지 학생까지 입력 시마다 총점이 갱신되는 거니까 calc()를 쓰면 안되고
				// 선택한 애만 수정이 되어야하니까 calc 에 choice_idx 인자가 들어간 거를 하나 더 만들어야 한다.
				// calc();
				
				choice_calc(choice_idx);			// 선택 계산
				
				System.out.printf("수정된 %s학생의 정보는 다음과 같습니다.\n", name[choice_idx]);
				data_print(choice_idx);
	...
	public void delete() {
		...
		while(true) {
			...
			if(duplicate_name_check(delete_name)) {
				// 동명이인이 2명 이상일 때
				System.out.printf("다음과 같이 %s 이름을 가진 학생이 둘 이상 존재합니다.\n", delete_name);
				System.out.printf("번호\t성명\t국어\t영어\t수학\t총점\t평균\t학점\t등수\n");

				// 동명이인의 인원수 구해서 그걸 출력범위로 사용해서 동명이인 리스트 출력할 것.
				count = duplicate_name_count(delete_name);
				
				// 원래 데이터에서 동명이인이 존재하는 인덱스 추출
				save_index = duplicate_name_idx(count, delete_name);
				
				// 동명이인 리스트 보여주고 번호 선택까지 한 후 해당 번호가 원래 데이터에서 몇 번 인덱스인지까지 구하기
				choice_idx = same_name_print(save_index, count, delete_name);
				
				for(i=choice_idx;i<size-1;i++) {		// 2
					name[i] = name[i+1];
					kor[i] = kor[i+1];
					eng[i] = eng[i+1];
					math[i] = math[i+1];
				...
				System.out.println("----- 검색한 학생 삭제를 진행합니다. -----");
				System.out.println("----- 삭제 완료 -----");
	...
	public void choice_calc(int choice_idx) {
		total[choice_idx] = kor[choice_idx] + eng[choice_idx] + math[choice_idx];
		avg[choice_idx] = total[choice_idx]/3.0;
		
		if(avg[choice_idx]>=90 && avg[choice_idx]<=100)
			grade[choice_idx] = "A";
		else if (avg[choice_idx]>=80 && avg[choice_idx]<90)
			grade[choice_idx] = "B";
		else if (avg[choice_idx]>=70 && avg[choice_idx]<80)
			grade[top] = "C";
		else if (avg[choice_idx]>=60 && avg[choice_idx]<70)
			grade[top] = "D";
		else
			grade[top] = "F";
		
		if(avg[choice_idx]%10>=7 && avg[choice_idx]>=60 || avg[choice_idx]==100)
			grade[choice_idx] = grade[top] + "+";
		else if (avg[choice_idx]%10<=3 && avg[choice_idx]>=60)
			grade[choice_idx] = grade[choice_idx] + "-";
		
		// 순위 계산
		setRank();
	}
	
	public int same_name_print(int[] save_index, int count, String search_name) {
		// save_index : 원래 데이터에서 동명이인이 존재하는 인덱스
		// count : 동명이인 인원수
		int i=0;
		...
		System.out.println("작업을 원하는 학생의 번호를 선택해주세요.");
