...
public class test_GUI_takgeun2 extends JFrame {
	...
	public test_GUI_takgeun2() {
	    ...
	    Image img = toolkit.getImage("C:\\SIAT\\java\\workspace\\SIATBankCard_sequence_ver\\SiatLogo.jpg");
	    setIconImage(img);
		...
		JButton modifyBtn = new JButton("카드정보수정");
		...
		// 카드수정
		// 수정 버튼 클릭 시 새 창이 뜨게 할 것
		// 다이얼로그 생성
		// JButton modifyBtn = new JButton("카드정보수정");
		MyDialog dialog = new MyDialog(this, "Test Dialog", rowIndex);
	...
	class MyDialog extends JDialog {
		...
		public MyDialog(JFrame frame, String title, int rowIndex) {
			//super(frame, title);
			setTitle("카드정보 검색하기");
			setSize(700, 500);
			setLocation(300, 100);
			...
			// 왼쪽 큰 패널 세팅
			JPanel p_west = new JPanel();
			p_west.setBorder(new TitledBorder(new LineBorder(Color.black, 1), "수정할 내용"));
			p_west.setPreferredSize(new Dimension(300, 480));

			// 오른쪽 큰 패널 세팅
			JPanel p_east = new JPanel();
			p_east.setBorder(new TitledBorder(new LineBorder(Color.black, 1), "수정 결과"));
			p_east.setPreferredSize(new Dimension(1000, 480));
			...
			// 하단 버튼 객체 모음
			JPanel btnWrap = new JPanel();
			JButton statusBtn = new JButton("정보 조회하기");
			JButton modifyBtn = new JButton("정보 수정하기");
			...
			modifyBtn.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					rows[0] = (String) table.getValueAt(rowIndex, 0);
					// 임시로 0을 넣을 것. 나중에 전체 조회할 때 인덱스 수를 추출해서 rows[인덱스수] 에 대입하면 될 듯 --> 해결
					rows[1] = cardNametf.getText();
					if (cardType1.isSelected())
						rows[2] = "신용카드";
					...
					// 데이터타입 조율하면서 변수 만든 후 Dao modify 메소드 호출
					int no = Integer.parseInt(rows[0]);
					String cardName = rows[1];
					String cardType = rows[2];
					int annualFee = Integer.parseInt(rows[3]);
